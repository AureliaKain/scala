...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    lazy val maxLeft = left.mostRetweeted
    lazy val maxRight = right.mostRetweeted
    if( !left.isEmpty && maxLeft.retweets > elem.retweets )
      if( !right.isEmpty && maxRight.retweets > maxLeft.retweets )
        maxRight
      else
        maxLeft
    else if( !right.isEmpty && maxRight.retweets > elem.retweets )
      maxRight
    else
      elem
  }
