 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of an empty list (0pts)`: Unit = {
    assert(sum(List()) == 0)
  }

  @Test def `sum of a single number (0pts)`: Unit = {
    assert(sum(List(1)) == 1)
  }
  ...
  @Test def `max of an empty list (0pts)`: Unit = {
    assert(max(List()) == java.util.NoSuchElementException)
  }

  @Test def `max of same elements of a list (0pts)`: Unit = {
    assert(max(List(1,2,2,2)) == 2)
  }
